...
- **Invoices** — matches the printed Fatoma invoice design (navy/gold): multiple vehicles per invoice, each row with the car's photo, name, package label, bulleted services, and before/after-discount totals; grand-total bars, payment details box, thank-you script, authorized signature, an uploadable constant footer picture, and the tagline bar. Records can be backdated — the invoice date, and the payment acknowledgment date, are both settable. Auto-numbered in the owner's format (FTM-250601 = year 25, month 06, invoice 01; resets monthly) or plain sequential. Discounts per vehicle and/or invoice-wide (amount or percent); duplicate any invoice in one tap
