...
- **Jobs** — schedule with date/time, add multiple vehicles for the same visit (e.g. a client dropping off two or three cars), pick packages/services/products per vehicle, add custom line items or a price override, track status (scheduled → in progress → completed), then generate a single multi-vehicle invoice in one tap
...
- **Invoices** — on the phone screen you get a clean, scrollable card summary (client, each vehicle with its services, totals, payment info); tapping Print or exporting a PDF renders the full printed Fatoma design (navy/gold) instead — multiple vehicles per invoice, each with the car's photo, name, package label, bulleted services, and before/after-discount totals, grand-total bars, payment details box, thank-you script, authorized signature, and the tagline bar. Records can be backdated — the invoice date, and the payment acknowledgment date, are both settable. Auto-numbered in the owner's format (FTM-250601 = year 25, month 06, invoice 01; resets monthly) or plain sequential. Discounts per vehicle and/or invoice-wide (amount or percent); duplicate any invoice in one tap
